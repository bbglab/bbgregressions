...
def do_regression_analysis(info_row, pdf,
    ...
    try:
        ...
        print("A dataframe with the totals of this metric does not exist. Continuing without including this analysis.")
    ...
    metric_predictors_df = metric_df.merge(predictors_df, right_index = True, left_index = True, how = "left")
    if totals_included:
        metric_predictors_df = metric_predictors_df.merge(metric_total_df, right_index = True, left_index = True, how = "left")
    ...
    if total_plot: # TODO: check this one is working
        responses4plot_dict = {}
        total_columns = [col for col in metric_predictors_df.columns if "total" in col and "depth" not in col]
    ...
    for mode in plotting_info_dict:
        ...
        if "predictors2test" not in pvals_df.columns:
            ...
            # Function to update the tests (TODO: change this to a simpler version, this is chatgpt)
            def update_tests():
                changed = False
            ...
            if info_row["covariables_multi"]:
                print(f"Adding mandatory covariables to the multivariate analysis: {info_row['covariables_multi']}")
                predictors_multi.extend(info_row["covariables_multi"])
                predictors4plot_multi.update(info_row["covariables_multi4plot"])
                predictors2plot_multi = predictors_multi
                for test in multi_tests:
                    multi_tests[test].extend(info_row["covariables_multi"])
                    if "depth" in multi_tests[test]:
                        multi_tests[test] = [f"{test.split('_')[0]}_{elem}" if elem == "depth" else elem for elem in multi_tests[test]]
            
            print(predictors_multi)
            print(predictors4plot_multi)
            print(predictors2plot_multi)
            print(multi_tests)
...
def main(config_file, pdf_path,
            ...
            for file in os.listdir(method_directory):
                if all(
                    ...
                    if "total_gene" not in file and "total_sample" not in file: # keep a single file per metric
                        ...
                        if config["variables"]["covariables_multi"]:
                            new_row["covariables_multi"] = [var for var in json.loads(config["variables"]["covariables_multi"]).keys()]
                            new_row["covariables_multi4plot"] = json.loads(config["variables"]["covariables_multi"])
                        else:
                            new_row["covariables_multi"] = []
                            new_row["covariables_multi4plot"] = {}
    ...
    if joining_rules:
        ...
        config_df = config_df.groupby("joining_rule", dropna = False).agg({'method': lambda x: x.iloc[0] if not x.isnull().all() else np.nan, # choose first instance, as it is always the same
                                                        ...
                                                        'covariables_multi': lambda x: x.iloc[0] if not x.isnull().all() else np.nan, 
                                                        'covariables_multi4plot': lambda x: x.iloc[0] if not x.isnull().all() else np.nan, 
